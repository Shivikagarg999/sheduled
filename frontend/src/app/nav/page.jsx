...
import { FaGlobe, FaChevronDown, FaBars, FaTimes } from 'react-icons/fa'; // make sure you're importing these icons
...
export default function Nav() {
  ...
  return (
    <motion.nav
      initial="hidden"
      animate="visible"
      className={`fixed top-6 left-4 right-4 mx-auto max-w-6xl bg-white rounded-xl shadow-lg z-50 transition-all duration-300 ${scrolled ? 'py-2 border-gray-300' : 'py-3 border-gray-300'} border-2 px-6 flex justify-between items-center backdrop-blur-sm bg-opacity-90`}
    >
      {/* Logo */}
      <div className="flex items-center space-x-4">
        <motion.div
          ...
          className="w-17 p-6 h-10 bg-gradient-to-br from-blue-500 to-blue-700 rounded-lg flex items-center justify-center text-white font-bold"
        >
          Logo
        </motion.div>
        ...
        <ul className="hidden md:flex space-x-9 text-gray-700 font-medium text-sm">
          ...
          <Link href="/send-parcel">
            <motion.li
              whileHover={{ scale: 1.05 }}
              className={`cursor-pointer hover:text-blue-600 transition-colors relative group ${isActive('/send-parcel') ? 'text-blue-600 font-semibold' : ''}`}
            >
              Send Parcel
              <motion.span
                className={`absolute bottom-0 left-0 h-0.5 bg-blue-600 transition-all ${isActive('/send-parcel') ? 'w-full' : 'w-0 group-hover:w-full'}`}
                layoutId="underline"
              />
            </motion.li>
          </Link>
